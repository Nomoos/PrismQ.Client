...
class CustomHandlers {
    ...
    #[OA\Post(
        ...
        requestBody: new OA\RequestBody(
            ...
            content: new OA\JsonContent(
                required: ['worker_id'],
                properties: [
                    new OA\Property(property: 'worker_id', type: 'string', example: 'worker-001', description: 'Unique worker identifier'),
                    new OA\Property(property: 'name', type: 'string', example: 'Worker 1', description: 'Human-readable worker name'),
                    new OA\Property(property: 'status', type: 'string', enum: ['active', 'idle', 'offline'], example: 'active', description: 'Worker status'),
                    new OA\Property(
                        property: 'metadata',
                        type: 'object',
                        description: 'Additional worker metadata',
                        example: ['version' => '1.0.0', 'capabilities' => ['task-type-1', 'task-type-2']]
                    )
                ]
            )
        ),
    ...
    public function worker_register($requestData, $config) {
        ...
        $stmt = $this->db->prepare("SELECT id FROM workers WHERE worker_id = ?");
        ...
        if ($existing) {
            ...
            $stmt = $this->db->prepare(
                "UPDATE workers SET name = ?, status = ?, metadata_json = ?, last_heartbeat = NOW(), updated_at = NOW() 
                 WHERE worker_id = ?"
            );
            $stmt->execute([$name, $status, $metadata, $worker_id]);
            ...
            // Create new worker
            $stmt = $this->db->prepare(
                "INSERT INTO workers (worker_id, name, status, metadata_json, last_heartbeat) 
                 VALUES (?, ?, ?, ?, NOW())"
            );
            $stmt->execute([$worker_id, $name, $status, $metadata]);
        ...
        responses: [
            new OA\Response(
                ...
                content: new OA\JsonContent(
                    properties: [
                        new OA\Property(property: 'worker_id', type: 'string', example: 'worker-001'),
                        new OA\Property(property: 'total_claimed', type: 'integer', example: 150),
                        new OA\Property(property: 'total_completed', type: 'integer', example: 145),
                        new OA\Property(property: 'total_failed', type: 'integer', example: 5),
                        new OA\Property(property: 'currently_claimed', type: 'integer', example: 2),
                        new OA\Property(property: 'success_rate', type: 'number', format: 'float', example: 96.7)
                    ]
                )
            ),
    ...
    public function worker_stats($requestData, $config) {
        $worker_id = $requestData['path']['worker_id'] ?? null;
        ...
        $stmt = $this->db->prepare("
            SELECT 
                COUNT(CASE WHEN status IN ('claimed', 'completed', 'failed') THEN 1 END) as total_claimed,
                COUNT(CASE WHEN status = 'completed' THEN 1 END) as total_completed,
                COUNT(CASE WHEN status = 'failed' THEN 1 END) as total_failed,
                COUNT(CASE WHEN status = 'claimed' THEN 1 END) as currently_claimed
            FROM tasks 
            WHERE claimed_by = ?
        ");
        $stmt->execute([$worker_id]);
        ...
        $success_rate = $total_finished > 0 
            ? round(($stats['total_completed'] / $total_finished) * 100, 1) 
            : 0;
        
        return [
            'worker_id' => $worker_id,
            'total_claimed' => (int)$stats['total_claimed'],
            'total_completed' => (int)$stats['total_completed'],
            'total_failed' => (int)$stats['total_failed'],
            'currently_claimed' => (int)$stats['currently_claimed'],
